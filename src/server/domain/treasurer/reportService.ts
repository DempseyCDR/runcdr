...
export type TreasurerReport = {
  ...
  expenses: {
    ...
    /**
     * Feature 085 (FR-008): did we pay everyone? What the evening's bookings came to, what was actually
     * paid (live per-line amounts, voids excluded), and what is still owed. The per-payment notes say
     * whether a payment was short; this says whether one is missing altogether.
     */
    reconciliation: { booked: number; paid: number; outstanding: number };
  };
...
export async function assembleTreasurerReport(
  ...
  // the booking's expected pay. A payment's role comes from the performer type of a booking it settles —
  // the one performed by the payee if present (the normal/backfilled case), else the first linked booking
  // (an aggregated check across types). Booked pay feeds the reconciliation.
  ...
  const reconciliation = (() => {
    ...
    return {
      booked: centsToDollars(r.expectedCents),
      paid: centsToDollars(r.actualCents),
      // Positive when someone booked is still owed; negative would mean we overpaid.
      outstanding: centsToDollars(r.expectedCents - r.actualCents),
    };
  })();
